...
const Renderer = (() => {
  ...
  function _updateCamera(localPlayer) {
    ...
    // Target zoom: shrinks as worm grows (longer = more zoomed out)
    const targetZoom = 1 / (1 + localPlayer.length * 0.0018);
    const clampedZoom = Math.max(0.2, Math.min(1.2, targetZoom));
